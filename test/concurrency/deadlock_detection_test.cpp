...
TEST(LockManagerDeadlockDetectionTest, CycleTest2) {
  ...
  lock_mgr.AddEdge(1, 4);
  lock_mgr.AddEdge(1, 5);
  lock_mgr.AddEdge(4, 3);
  lock_mgr.AddEdge(4, 6);
  lock_mgr.AddEdge(4, 9);
  lock_mgr.AddEdge(9, 1);
  lock_mgr.AddEdge(5, 6);
  lock_mgr.AddEdge(3, 8);
  lock_mgr.AddEdge(6, 7);
  lock_mgr.AddEdge(8, 1);
  lock_mgr.AddEdge(7, 4);
  lock_mgr.AddEdge(2, 8);
  lock_mgr.AddEdge(10, 11);
  lock_mgr.AddEdge(11, 12);
  lock_mgr.AddEdge(11, 13);
  lock_mgr.AddEdge(13, 10);
  lock_mgr.AddEdge(12, 10);
  txn_id_t txn_id;
  EXPECT_EQ(true, lock_mgr.HasCycle(&txn_id));
  EXPECT_EQ(8, txn_id);
  lock_mgr.RemoveEdge(2, 8);
  lock_mgr.RemoveEdge(3, 8);
  EXPECT_EQ(true, lock_mgr.HasCycle(&txn_id));
  EXPECT_EQ(7, txn_id);
  lock_mgr.RemoveEdge(6, 7);
  EXPECT_EQ(true, lock_mgr.HasCycle(&txn_id));
  EXPECT_EQ(9, txn_id);
  lock_mgr.RemoveEdge(4, 9);
  EXPECT_EQ(true, lock_mgr.HasCycle(&txn_id));
  EXPECT_EQ(12, txn_id);
  lock_mgr.RemoveEdge(11, 12);
  EXPECT_EQ(true, lock_mgr.HasCycle(&txn_id));
  EXPECT_EQ(13, txn_id);
  lock_mgr.RemoveEdge(11, 13);
